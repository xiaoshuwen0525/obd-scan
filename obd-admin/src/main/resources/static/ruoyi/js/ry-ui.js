...
(function ($) {
    $.extend({
        ...
        table: {
            ...
            // 导出脱敏数据
            exportDesensitizationExcel: function (formId) {
                table.set();
                $.modal.confirm("确定导出所有" + table.options.modalName + "吗？", function () {
                    ...
                    dataParam.push({"name": "isAsc", "value": params.sortOrder});
                    $.modal.loading("正在导出数据，请稍后...");
                    $.post(table.options.desensitizationUrl, dataParam, function (result) {
                        if (result.code == web_status.SUCCESS) {
                            window.location.href = ctx + "common/download?fileName=" + encodeURI(result.msg) + "&delete=" + true;
                        } else if (result.code == web_status.WARNING) {
                            $.modal.alertWarning(result.msg)
            ...
            importPortExcel: function (formId) {
                ...
                layer.open({
                    ...
                    maxmin: true,
                    shade: 0.3,
                    title: '导入' + table.options.modalPortName + '数据',
                    content: $('#' + currentId).html(),
                    btn: ['<i class="fa fa-check"></i> 导入', '<i class="fa fa-remove"></i> 取消'],
                    // 弹层外区域关闭
                    shadeClose: false,
                    btn1: function (index, layero) {
                        ...
                        var formData = new FormData(layero.find('form')[0]);
                        $.ajax({
                            url: table.options.importPortUrl,
                            data: formData,
                            cache: false,
                            contentType: false,
                            processData: false,
            ...
            // 导入数据
            importXls: function (importUrl, title, orderType, formId) {
                table.set();
                var currentId = $.common.isEmpty(formId) ? 'importTpl' : formId;
                layer.open({
                    type: 1,
                            ...
                            success: function (result) {
                                if (result.code == web_status.SUCCESS) {
                                    ...
                                    $.modal.confirm(result.msg, function () {
                                        var tem = result.msg.match('\\d+\\d{5}')
                                        $.table.detail(tem[0], orderType)
                                    });
                                    $.table.refresh();
                                } else if (result.code == web_status.WARNING) {
                                    layer.close(index);
            ...
            // 下载模板
            importTemplateByUrl: function (url) {
                table.set();
            ...
            detail: function (id, orderType) {
                if (orderType == 'batch_account_discounts') {
                    var url = prefix + '/batch_account_discounts_detail/' + id;
                    $.modal.openTab("政企优惠受理批量开户", url);
                } else {
                    var url = prefix + '/batch_account_private_detail/' + id;
                    $.modal.openTab("政企私人名批量开户", url);
                }
            },
            // 刷新表格
            ...
            selectColumnsDef: function (column) {
                var rows = $.map($("#" + table.options.id).bootstrapTable('getSelections'), function (row) {
                    return row[column];
                });
                if ($.common.isNotEmpty(table.options.rememberSelected) && table.options.rememberSelected) {
            ...
            // 回显数据字典
            selectDictLabelByCode: function (datas, value) {
                var actions = [];
                $.each(datas, function (index, dict) {
        ...
        modal: {
            ...
            // 弹出绑定手机号指定宽度
            openBindPhone: function (title, url, width, height, callback) {
                //如果是移动端，就使用自适应大小弹窗
                if (navigator.userAgent.match(/(iPhone|iPod|Android|ios)/i)) {
                    width = 'auto';
                    height = 'auto';
        ...
        operate: {
            ...
            redeploy: function (orderState) {
                ...
                var rows = $.table.selectColumns("orderNo");
                if (rows.length == 0) {
                    $.modal.alertWarning("请至少选择一条记录");
                    return;
                }
                ...
                if (!orderState) {
                    var url = table.options.redeployUrl.replace("{orderNos}", rows.join());
                    $.modal.open("转派订单" + table.options.modalName, url, "500", "500");
                    return;
                }

                var tem = $.table.selectColumns("orderStatus");
                console.log("选中的状态是：" + tem)
                for (t in tem) {
                    if (orderState.indexOf(tem[t]) == -1) {
                        $.modal.alertWarning("订单状态不允许转派！");
                        return;
                    }
                }
                var url = table.options.redeployUrl.replace("{orderNos}", rows.join());
            ...
            /* lcx */
            redeployOtherOrders: function (orderState) {
                table.set();
                var rows = $.table.selectColumns("orderNo");
                var version = $.table.selectColumnsDef("version");
                ...
                if (!orderState) {
                    var url = table.options.redeployUrl.replace("{orderNos}", rows.join()).replace("{versions}", version.join());
                    $.modal.open("转派订单" + table.options.modalName, url, "500", "500");
                    return;
                }
            ...
            rollbackOrders: function (data) {
                ...
                var rowStatus = $.table.selectColumns("orderStatus");

                for (var y in rowStatus) {

                    if (data.indexOf(rowStatus[y]) == -1) {
                        $.modal.alertWarning("订单状态不允许退单！");
                        return;
                    }
                ...
                var url = table.options.rollbackUrl.replace("{orderNos}", rows.join());
                $.modal.open("退单订单 — " + table.options.modalName, url, "500", "500");
            },
            //订单归档
            pigeonholeOrders: function (data) {
                table.set();
                var rows = $.table.selectColumns("orderNo");
                if (rows.length == 0) {
                    $.modal.alertWarning("请至少选择一条记录");
                ...
                for (var y in rowStatus) {
                    ...
                    if (data.indexOf(rowStatus[y]) == -1) {
                        $.modal.alertWarning("订单状态不允许归档！");
                        return;
                    }

                }

                var url = table.options.pigeonholeUrl.replace("{orderNos}", rows.join());

                 $.modal.open("归档 — " + table.options.modalName, url, "500", "500");


            },
            //订单归档
            editPigeonholeOrders: function (orderNo) {

                var url = table.options.pigeonholeUrl.replace("{orderNos}", orderNo);

                $.modal.open("归档 — " + table.options.modalName, url, "500", "500");

            },
            //弹出 订单作废 框
            cancellationOrders: function (data) {
                table.set();
                debugger
                var rows = $.table.selectColumns("orderNo");
                if (rows.length == 0) {
                ...
                for (var y in rowStatus) {
                    ...
                    if (data.indexOf(rowStatus[y]) == -1) {
                        $.modal.alertWarning("订单状态不允许作废！");
                        return;
                    }

                }

                var url = table.options.cancellationUrl.replace("{orderNos}", rows.join());
                $.modal.open("作废订单 — " + table.options.modalName, url, "500", "500");
            },
            // 修改信息
            orderEdit: function (state) {
                table.set();
                var rowOrderNo = $.table.selectColumns("orderStatus");

                if (state.indexOf(rowOrderNo) > -1) {
                    $.modal.open("修改" + table.options.modalName, $.operate.editUrl());
                } else {
                    $.modal.alertWarning("订单状态不允许修改！");
                }

            },
            //弹出日志框
            showLogTable: function (orderNo) {
                var url = table.options.openLogUrl.replace("{orderNo}", orderNo);
                $.modal.open("查看日志 — " + table.options.modalName, url, 750, 650);
            },
            // 清空信息
            clean: function () {
                table.set();
